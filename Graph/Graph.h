...
void CreateGraph(struct Graph* g, size_t vertices){
    ...
    g->AdjacencyList = (struct LinkedList **) malloc(sizeof(struct LinkedList *)*(vertices+1));
    
    for(size_t i=0; i<g->vertices+1; i++){
        g->AdjacencyList[i] = (struct LinkedList *) malloc(sizeof(struct LinkedList));
        Initialize(g->AdjacencyList[i]);
    }
}
...
int AddEdge(struct Graph *g, int src, int des){
    ...
    InsertSorted(g->AdjacencyList[src], des);
    //for undirected graph
    InsertSorted(g->AdjacencyList[des], src);
    return 0;
}
...
void BFS(struct Graph *g, int root){
    ...
    while(!IsEmpty(&q)){
        int curr = DeQueue(&q);
        struct Node * temp = g->AdjacencyList[curr]->head;
        while(temp != NULL){
...
void DFS(struct Graph *g, int root){
    ...
    while(!isEmpty(&s)){
        int curr = Pop(&s);
        temp = g->AdjacencyList[curr]->head;
        ...
        while(temp){
            ...
            if(temp){
                ...
                temp = g->AdjacencyList[temp->value]->head;
            }
        }
    }
    printf("\n");
...
void FreeGraph(struct Graph *g){
    for(size_t i=0; i<=g->vertices; i++){
        freeLinkedlist(g->AdjacencyList[i]);
